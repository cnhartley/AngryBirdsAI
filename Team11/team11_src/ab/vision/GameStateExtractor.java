...
public class GameStateExtractor {
	...
	private final void loadResources() {
		try {
			_mainmenu = loadResource("mainmenu.png");
			_episodemenu = loadResource("episodemenu.png");
			_levelselection = loadResource("levelselection.png");
			_loading = loadResource("loading.png");
			_loading2 = loadResource("loading2.png");
			_gamewon1 = loadResource("gamewon1.png");
			_gamewon2 = loadResource("gamewon2.png");
			_gamelost = loadResource("gamelost.png");
			_endGame0 = loadResource("0endScreen.png");
			_endGame1 = loadResource("1endScreen.png");
			_endGame2 = loadResource("2endScreen.png");
			_endGame3 = loadResource("3endScreen.png");
			_endGame4 = loadResource("4endScreen.png");
			_endGame5 = loadResource("5endScreen.png");
			_endGame6 = loadResource("6endScreen.png");
			_endGame7 = loadResource("7endScreen.png");
			_endGame8 = loadResource("8endScreen.png");
			_endGame9 = loadResource("9endScreen.png");
			
			loaded = true;
		}
		catch (IOException e) {
			System.err.println(" <!> Failed to load resource(s): " + e);
			System.exit(102);
		}
	}
	
	private final BufferedImage loadResource(String relPath)
			throws IOException
	{
		return ImageIO.read(getClass().getResource("resources/" + relPath));
	}
	...
	private static BufferedImage extractNumber(BufferedImage image) {
		...
		int mask[][] = createImageMask(image, 255, 16);
	...
	private static final int[][] createImageMask(BufferedImage img, int colorMask, int shift) {
		final int height = img != null ? img.getHeight() : 0;
		final int width = img != null ? img.getWidth() : 0;
		final int cmask = shift > 0 ? colorMask << shift : colorMask;
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				color = (img.getRGB(x, y) & cmask) >> shift;
				mask[y][x] = color > 192 ? 1 : -1;
			}
		}
	...
	public int getScoreEndGame(BufferedImage screenshot) {
		...
		// extract characters
		int mask[][] = createImageMask(scoreImage, 255, 16);
		/*int mask[][] = new int[scoreImage.getHeight()][scoreImage.getWidth()];
		for (int y = 0; y < scoreImage.getHeight(); y++) {
			for (int x = 0; x < scoreImage.getWidth(); x++) {
				final int colour = scoreImage.getRGB(x, y);
		...
		for (int i = 0; i < letters.length; i++) {
			...
			int diff;
			for (int j = 0; j < 10; j++) {
		        diff = getPixelDifference(letterImage, endGameNumberTemplates[j]);
				if (diff < minDiff){
					minDiff = diff;
					value = j;
				}
